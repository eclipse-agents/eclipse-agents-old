...
public class AcpClaudePreferencePage extends PreferencePage implements 
    ...
    final String claudePreferenceId = new ClaudeService().getStartupCommandPreferenceId();
    ...
    public AcpClaudePreferencePage() {
        super();
    ...
    protected Control createContents(Composite ancestor) {
        ...
        for (IAgentService service: AgentController.instance().getAgents()) {
            if (service instanceof ClaudeService) {
                installLocation.setText(((ClaudeService)service).getAgentsNodeDirectory().getAbsolutePath());
            }
        }
        return parent;
    }
        ...
        for (IAgentService service: AgentController.instance().getAgents()) {
            if (service instanceof ClaudeService) {
                if (service.isRunning() && service.getInitializeResponse() != null) {
                    ...
                    buffer.append("Claude CLI Features:");
            ...
            Activator.getDisplay().asyncExec(()->{
                installVersion.setText(((ClaudeService)service).getVersion());
            });
        }
    }
    ...
    private void loadPreferences() {
        ...
        input.setText(store.getString(claudePreferenceId));
        useLocalInstall.setSelection(store.getString(claudePreferenceId).equals(store.getDefaultString(claudePreferenceId)));
        targetVersion.setText(store.getString(P_ACP_CLAUDE_VERSION));
    }
    ...
    private void savePreferences() {
        ...
        store.setValue(claudePreferenceId, preference);
        store.setValue(P_ACP_CLAUDE_VERSION, targetVersion.getText());
    }
    ...
    protected void performDefaults() {
        IPreferenceStore store = getPreferenceStore();
        input.setText(store.getDefaultString(claudePreferenceId));
        targetVersion.setText(store.getDefaultString(P_ACP_CLAUDE_VERSION));
    ...
    public void widgetSelected(SelectionEvent event) {
        if (event.getSource() == start) {
            ...
            if (useLocalInstall.getSelection()) {
                input.setText(getPreferenceStore().getDefaultString(claudePreferenceId));
            } else {
                input.setText("claude\n--ide");
            }
            parent.layout(true);
        }
